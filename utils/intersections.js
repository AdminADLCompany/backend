...
exports.handleIntersection = catchAsyncErrors(
  async (process, items, userId, isEdit, args = {}) => {
    const { rowDataId, row, rowId, previousItems } = args;
    if (!isEdit) {
        ...
        if (moveToItem && moveToItem.value === "Rework") {
          ...
          if (!rejectionProcess) {
            throw new ErrorHandler(
              "Rejection Process (MR/R/003) not found",
              404
            );
          }
      ...
      // ---- PR/R/003A → QA/R/003A ---- Procurement → Inspection
        ...
        if (!inspectionProcess)
          throw new ErrorHandler(
            "Inspection Process (QA/R/003) not found",
            404
          );
        if (!procurementProcess)
          throw new ErrorHandler(
            "Procurement Process (PR/R/003) not found",
            404
          );
      ...
      // ---- MR/R/001 → PR/R/003 ---- Production Plan → Procurement Register
        ...
        for (const productRow of matchingProducts) {
          const detailingIds =
            productRow.items.find((i) => i.key === "DETAILING PRODUCT")
              ?.value || [];
      ...
      // ---- PR/R/002 → ST/R/006 ---- Item List → Stock Data
        ...
        if (!stockDataProcess)
          throw new ErrorHandler(
            "Stock Data Process (ST/R/006) not found",
            404
          );
      ...
      // ---- MR/R/002A -> MR/R/002 (OEE Calculation) ----
        ...
        const totalTime =
          Number(
            productionReportRow.items.find((i) => i.key === "PLAN")?.value
          ) *
          Number(
        ...
        const planItem = productionReportRow.items.find(
          (i) => i.key === "PLAN"
        );
      ...
      if (process.processId === "MR/R/003B") {
        ...
        if (moveToItem && moveToItem.value === "Rework") {
          ...
          const reworkReport = await Process.findOne({
            processId: "MR/R/003A",
          });
              ...
              const reworkRow = {
                items: [
                  ...
                  },
                  {
                    key: "STATUS",
                    value:
                      items.find((i) => i.key === "ACTION PLAN STATUS")
                        ?.value || "",
                    process: "value",
                  },
                ],
      ...
      // ---- MS/R/005 → MS/R/006 ----
        ...
        if (moveToItem && moveToItem.value === "ORDER") {
          ...
          const matchedRow = storeRegisterProcess.data.find((itemRow) => {
            ...
            return (
              rowItemCode === items.find((i) => i.key === "PART NO")?.value
            );
          });
          ...
          const storeRegisterRow = {
            items: [
              ...
              {
                key: "ITEM CODE",
                value: matchedRow.items.find((i) => i.key === "ITEM CODE")
                  ?.value,
                process: "value",
              },
              {
                key: "ITEM NAME",
                value: matchedRow.items.find((i) => i.key === "ITEM NAME")
                  ?.value,
                process: "value",
              },
              {
                key: "VENDOR / CUSTOMER",
                value: matchedRow.items.find(
                  (i) => i.key === "VENDOR / CUSTOMER"
                )?.value,
                process: "value",
              },
              {
          ...
          if (stockDataProcess) {
            ...
            if (existingStockRow) {
              // 🟠 Update existing stock entry (OUT operation)
              const inItem = existingStockRow.items.find((i) => i.key === "IN");
              const outItem = existingStockRow.items.find(
                (i) => i.key === "OUT"
              );
              const stockItem = existingStockRow.items.find(
                (i) => i.key === "STOCK"
      ...
      // ---- DD/R/010
        ...
        const protoItem = correspondingItem.items.find(
          (i) => i.key === "PROTO"
        );
